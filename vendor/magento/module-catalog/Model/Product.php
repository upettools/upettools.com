    ...
    ProductInterface
{
    ...
    public function getPrice()
    {
		// if ($this->_calculatePrice || !$this->getData(self::PRICE)) {
			...
			// // $productRepository = $objectManager->get('\Magento\Catalog\Model\ProductFactory')->create();
			// $product = $productRepository->getById($this->getId());
			// //print_r($product->getWeight());exit;
			// $priceHelper = $objectManager->create('Magento\Framework\Pricing\Helper\Data'); // Instance of Pricing Helper
			// $price =  113/6.3; //Your Price
			// $formattedPrice = $priceHelper->currency($price, false, false);
			// //$freeShippingPrice = $this->_getData('special_price') + $formattedPrice*($product->getWeight());
			// //return $freeShippingPrice;
			
			// //print_r($product);exit;
            // return $this->getPriceModel()->getPrice($this) + $formattedPrice*($product->getWeight());
        // } else {
            // return $this->getData(self::PRICE) + $formattedPrice*($product->getWeight());
        // }
        if ($this->_calculatePrice || !$this->getData(self::PRICE)) {
    ...
    public function getSpecialPrice()
    {
		...
		// $price =  113/6.3; //Your Price
		// $formattedPrice = $priceHelper->currency($price, false, false);
		// if($this->_getData('special_price')>0){
			// $freeShippingPrice = $this->_getData('special_price') + $formattedPrice*($product->getWeight());
		    // return $freeShippingPrice;
		// }else{
